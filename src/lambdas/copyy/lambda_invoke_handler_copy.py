    ...
    if context:
        if isinstance(context, dict):
            ...
            if context.get("additional_info"):
                ...
                print(f"✅ Added additional_info to prompt ({len(additional)} chars)")
        elif isinstance(context, str):
            infrastructure_context = f"\n\n**Infrastructure Context:**\n{context}\n"

    if infrastructure_context:
        print(f"📝 Total infrastructure context: {len(infrastructure_context)} chars")
    else:
        print(f"⚠️ No infrastructure context added to prompt")
...
def send_teams_workflow(
    ...
    """Send notification to Microsoft Teams via Power Automate workflow"""
    workflow_url = os.environ.get("TEAMS_WORKFLOW_URL")

    if not workflow_url:
        print("TEAMS_WORKFLOW_URL environment variable not set")
        return False

    # Clean text to avoid JSON parsing issues in adaptive cards
    def clean_text(text):
        if not text:
    ...
    # Extract summary from agent analysis (simple version)
    summary = clean_text(summarize_agent_analysis(agent_analysis))

    # Construct execute URL with query parameters
    full_execute_url = execute_url
    if execute_url and session_id:
        # Build query parameters for execution Lambda
        query_params = {
            'alert_id': alert_id,
            'session_id': session_id,
            'service_name': service_name,
        ...
        if s3_key:
            query_params['s3_key'] = s3_key

        # Encode and append to URL
        query_string = urllib.parse.urlencode(query_params)
        full_execute_url = f"{execute_url}?{query_string}"
        print(f"📋 Execute URL with params: {full_execute_url}")
    else:
        print(f"⚠️ Execute URL or session_id missing - executeUrl will be incomplete")

    # Build payload for Power Automate workflow
    payload = {
    ...
    print(f"📤 Sending to Teams Workflow: Alert={alert_id}, Service={service_name}")
    ...
    try:
        ...
        if response.status in [200, 202]:
            print(f"✅ Teams workflow notification sent (status: {response.status})")
            return True
        else:
            print(f"⚠️ Teams workflow returned status: {response.status}")
            return False

    except Exception as e:
        print(f"❌ Failed to send Teams workflow notification: {e}")
        return False
...
def lambda_handler(event, context):
    ...
    if service_context:
        if isinstance(service_context, dict):
            ...
            print(f"   - AWS Services: {len(service_context.get('aws_services', []))} items")
            print(f"   - Additional Info: {len(service_context.get('additional_info', ''))} chars")
        else:
