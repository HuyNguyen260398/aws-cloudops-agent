...
class AgentCoreConfigManager:
    ...
    def _deep_merge(
        ...
        Only overrides base values with non-empty override values.
        Empty strings, empty lists, and empty dicts are considered "no value"
        and won't override existing base values.
        """
        result = base.copy()
        ...
        for key, value in override.items():
            if (
                ...
                # Only override if the value is meaningful (not empty)
                if self._is_meaningful_value(value):
                    result[key] = value
                elif key not in result:
                    # If the key doesn't exist in base, still add it even if empty
                    result[key] = value
    ...
    def get_model_settings(self) -> Dict[str, Any]:
        ...
        return {
            "region_name": aws_config.get("region", "ap-southeast-1"),
        ...
        try:
            ...
            if self._validator is None:
                try:
                    ...
                    logger.warning("ConfigValidator not found, skipping validation")
                    return True
